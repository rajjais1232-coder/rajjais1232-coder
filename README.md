<!-- ========================= HEADER ========================= -->

<div align="center">

# 👋 Hi, I'm Raj Jaiswal

### 📊 Data Analyst | Business Intelligence | AI & Machine Learning

<p>
  <b>Turning Data into Insights • Building Intelligent Solutions • Solving Real-World Problems</b>
</p>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=00D9FF&center=true&vCenter=true&width=750&lines=Data+Analytics+%7C+Business+Intelligence;SQL+%7C+Python+%7C+Power+BI+%7C+Excel;Machine+Learning+%7C+AI+%7C+Data+Visualization;Always+Learning+%7C+Always+Building+%7C+Always+Improving" alt="Typing SVG" />

<br>

<a href="https://www.linkedin.com/in/raj-jaiswal-644782336">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
&nbsp;
<a href="https://github.com/rajjais1232-coder">
  <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
&nbsp;
<a href="mailto:rajjais1232@gmail.com">
  <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
&nbsp;
<a href="https://portfolio-website-8p9oi8tbn-rajjais1232-coders-projects.vercel.app">
  <img src="https://img.shields.io/badge/Portfolio-Visit-00C7B7?style=for-the-badge&logo=vercel&logoColor=white" />
</a>

</div>

---

<!-- ========================= ABOUT ========================= -->

## 👨‍💻 About Me

I'm **Raj Jaiswal**, a B.Tech Information Technology student at 
**Government Engineering College, Bilaspur**, passionate about **Data Analytics, Business Intelligence, Artificial Intelligence, and Machine Learning**.

I enjoy transforming raw and complex datasets into meaningful insights, interactive dashboards, analytical models, and practical business solutions.

### 🎯 What I Focus On

- 📊 Data Analytics & Business Intelligence
- 🧮 Advanced SQL & Database Analysis
- 📈 Power BI Dashboard Development
- 🐍 Python & Exploratory Data Analysis
- 📑 Excel & Power Query
- 🤖 Machine Learning & AI
- 💡 Business & Financial Analytics
- 🌾 Agriculture & Sustainability Analytics
- 🏥 Healthcare Analytics
- 🏦 Banking & Risk Analytics

---

## 🎓 Education

**Government Engineering College, Bilaspur**  
**B.Tech — Information Technology** | 2024 – 2028

**CGPA:** 7.5 / 10

**Academic Background**
- Class XII — 76.2%
- Class X — 80%

---

<!-- ========================= TECH STACK ========================= -->

## 🛠️ Technical Skills

### 📊 Data Analytics & BI

<p>
<img src="https://img.shields.io/badge/Excel-Advanced-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white"/>
<img src="https://img.shields.io/badge/Power%20BI-Advanced-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
<img src="https://img.shields.io/badge/DAX-Analytics-0078D4?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Power%20Query-ETL-742774?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Tableau-Visualization-E97627?style=for-the-badge&logo=tableau&logoColor=white"/>
</p>

### 🐍 Programming & Data Science

<p>
<img src="https://img.shields.io/badge/Python-3.x-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/Pandas-Data%20Analysis-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-Scientific%20Computing-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/Matplotlib-Visualization-11557C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Seaborn-Visualization-4C72B0?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Scikit--Learn-Machine%20Learning-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white"/>
</p>

### 🗄️ Databases & Cloud

<p>
<img src="https://img.shields.io/badge/SQL-Advanced-4479A1?style=for-the-badge"/>
<img src="https://img.shields.io/badge/MySQL-Database-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
<img src="https://img.shields.io/badge/Snowflake-Data%20Warehouse-29B5E8?style=for-the-badge&logo=snowflake&logoColor=white"/>
<img src="https://img.shields.io/badge/Microsoft%20Azure-Cloud-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white"/>
</p>

### ⚙️ Tools & Development

<p>
<img src="https://img.shields.io/badge/Git-Version%20Control-F05032?style=for-the-badge&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub-Code%20Hosting-181717?style=for-the-badge&logo=github&logoColor=white"/>
<img src="https://img.shields.io/badge/Jupyter-Notebook-F37626?style=for-the-badge&logo=jupyter&logoColor=white"/>
<img src="https://img.shields.io/badge/VS%20Code-IDE-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white"/>
</p>

---

<!-- ========================= EXPERIENCE ========================= -->

## 💼 Experience

### 📊 Data Analyst Intern — Zetheta Algorithms Private Limited
**Sep 2026 – Present | Remote**

- Working on financial data analytics and securitisation risk analytics.
- Developing Power BI dashboards and advanced DAX measures.
- Working with portfolio-level financial and risk metrics.
- Exploring PD, LGD, EAD and IFRS 9 ECL concepts.
- Building analytical views for portfolio monitoring and scenario analysis.

### 🤖 Applied AI Intern — IBM
**Aug 2026 – Sep 2026 | Remote**

- Worked on practical Artificial Intelligence concepts.
- Explored AI agents and AI-driven problem solving.
- Applied modern AI tools to real-world use cases.

### 🐍 Machine Learning Engineer Intern — RAYS IT & DESIGN WORLD
**Aug 2026 | Hybrid**

- Hands-on training in Python, Data Analytics and Machine Learning.
- Worked with data preprocessing, visualization and ML concepts.
- Built practical analytical and machine learning workflows.

### 📊 Big Data Analyst — AICTE
**Jun 2026 – Jul 2026 | Part-time**

- Worked on data-oriented analytical tasks.
- Applied data processing, analysis and visualization concepts.
- Developed practical experience with analytical workflows.

### 📈 Data Analyst Intern — Bluestock Fintech
**Apr 2026 – May 2026**

- Worked with Village API and NIFTY market data.
- Performed data collection, cleaning and preprocessing.
- Conducted exploratory data analysis and visualization.
- Converted analytical findings into business-oriented insights.

---

<!-- ========================= PROJECTS ========================= -->

## 🚀 Featured Projects

### 🏥 Healthcare Insurance Claims Analytics

**SQL • Excel • Python • Power BI • DAX**

End-to-end healthcare claims analytics project focused on:

- Claims and financial KPI analysis
- Patient demographics
- Provider performance
- Diagnosis analysis
- Claim approval/payment trends
- Outstanding claim analysis
- Interactive Power BI dashboards
- Business insights and data quality analysis

🔗 **Repository:**  
[Healthcare Insurance Claim Analysis](https://github.com/rajjais1232-coder/Healthcare-Insurance-Claim-Analysis)

---

### 🏦 Banking Customer Risk Analytics

**SQL • Python • Power BI • DAX**

Analytical project focused on:

- Customer segmentation
- Loan portfolio analysis
- Risk indicators
- Customer demographics
- Financial KPIs
- Loan performance
- Interactive Power BI dashboards
- Business intelligence reporting

🔗 **Repository:**  
[Banking Customer Risk Analytics](https://github.com/rajjais1232-coder/Banking-Risk-analytics)

---

### 🛒 E-Commerce Customer Intelligence

**SQL • Python • Excel • Power BI**

Business analytics project covering:

- Sales performance
- Revenue analysis
- Customer segmentation
- RFM analysis
- Customer churn
- Customer Lifetime Value
- Product performance
- KPI dashboards
- Business recommendations

---

### 🌾 Bio-Krishi AI

**Artificial Intelligence • Data Analytics • Agriculture**

An agriculture-focused AI concept designed around using technology and data to support farmers and make agricultural information more accessible.

The project explores the intersection of:

- Artificial Intelligence
- Agriculture
- Data
- Farmer-focused technology
- Decision support
- Sustainable innovation

---

### 💰 Securitisation Risk Analytics

**Power BI • DAX • Financial Analytics**

Advanced financial analytics project involving:

- Securitisation portfolio analysis
- Auto loan analytics
- Delinquency analysis
- DPD and roll-rate analysis
- IFRS 9 ECL concepts
- PD / LGD / EAD
- Waterfall analysis
- Stress testing
- Vintage analysis
- Investor-oriented reporting

---

<!-- ========================= ANALYTICS WORKFLOW ========================= -->

## 🔍 My Data Analytics Workflow

```text
Raw Data
    ↓
Data Collection
    ↓
Data Cleaning & Transformation
    ↓
SQL Analysis
    ↓
Exploratory Data Analysis
    ↓
Statistical Analysis
    ↓
KPI Development
    ↓
Dashboard Development
    ↓
Business Insights
    ↓
Data-Driven Decision Making


<!-- ========================= CERTIFICATIONS ========================= -->
🏆 Certifications & Programs
IBM — Make Agentic AI Work for You
IBM — Generative AI in Action
IBM — AI Fundamentals
Bluestock Fintech — Data Analyst Internship
RAYS IT & DESIGN WORLD — Python, Data Analytics & ML Training
AICTE — Big Data Analytics Internship/Training
Udemy — Data Analysis
AI-Shield Workshop
Business Model Competitions
Innovation & Hackathon Programs
<!-- ========================= ACHIEVEMENTS ========================= -->
🏅 Achievements & Activities
🚀 Participated in AGRITHON 2026
💡 Participated in innovation and business-model competitions
🤖 Worked on AI & Data Analytics projects
🏦 Built financial and banking analytics projects
🏥 Built healthcare analytics projects
🌾 Explored AI applications in agriculture
🏆 Participated in technical and innovation programs
🏐 State-Level Kabaddi Player
<!-- ========================= GITHUB STATS ========================= -->
📊 GitHub Analytics
<div align="center"> <img src="https://github-readme-stats.vercel.app/api?username=rajjais1232-coder&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" height="170"/> <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rajjais1232-coder&layout=compact&theme=tokyonight&hide_border=true" height="170"/>

<br><br>

<img src="https://streak-stats.demolab.com?user=rajjais1232-coder&theme=tokyonight&hide_border=true" /> </div>
<!-- ========================= CONTRIBUTIONS ========================= -->
🐍 Contribution Activity
<div align="center"> <img src="https://raw.githubusercontent.com/rajjais1232-coder/rajjais1232-coder/output/github-contribution-grid-snake.svg" alt="GitHub Contribution Snake"/> </div>
<!-- ========================= OPEN TO ========================= -->
🎯 Open To

I'm currently interested in opportunities related to:

📊 Data Analyst Internships
📈 Business Analyst Internships
💼 Business Intelligence
💰 Financial Data Analytics
🐍 Python & Data Analytics
🤖 AI & Machine Learning
🏦 Risk Analytics
🌾 Agriculture & Sustainability Analytics
🤝 Let's Connect
<div align="center">

If you're working on Data Analytics, Business Intelligence, AI, Machine Learning, Financial Analytics, or real-world technology projects, feel free to connect.

<br> <a href="https://www.linkedin.com/in/raj-jaiswal-644782336"> <img src="https://img.shields.io/badge/LinkedIn-Raj%20Jaiswal-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/> </a> <a href="https://github.com/rajjais1232-coder"> <img src="https://img.shields.io/badge/GitHub-rajjais1232--coder-181717?style=for-the-badge&logo=github&logoColor=white"/> </a> <a href="mailto:rajjais1232@gmail.com"> <img src="https://img.shields.io/badge/Email-rajjais1232%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/> </a>

<br><br>

💡 "Turning Data Into Insights, and Ideas Into Solutions."
</div>